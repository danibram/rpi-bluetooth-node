...
noble.on('discover', function(peripheral) {
    ...
    if (isSticker(peripheral)) {
        data = parseSticker(peripheral)
        data.type = 'sticker'
        delete peripheral._noble
        data.raw = peripheral
        console.log(data.type + ' "' + data.id + '" entered (RSSI ' + data.rssi + ') ' + new Date());
    } else if (isBeacon(peripheral)) {
        data.type = 'beacon'
        delete peripheral._noble
        data.raw = peripheral
        console.log(data.type + ' "' + data.id + '" entered (RSSI ' + data.rssi + ') ' + new Date());
...
noble.on('stateChange', function(state) {
    if (state === 'poweredOn') {
        ...
        console.log('Turn on the bluetooth please!')
        noble.stopScanning();
    }
});

isSticker = function(peripheral) {
    var manufacturerData = peripheral.advertisement.manufacturerData;

    return (manufacturerData &&
        EXPECTED_MANUFACTURER_DATA_LENGTH <= manufacturerData.length &&
        manufacturerData[0] === 0x5d && manufacturerData[1] === 0x01 &&
        manufacturerData[2] === 0x01);
}

isBeacon = function(peripheral) {
    var localName = peripheral.advertisement.localName;
    return ((localName === 'estimote' || localName === 'EST') &&
...
convertMotionStateDuration = function(raw) {
    var unit = (raw >> 6) & 0x03;
...
parseSticker = function(peripheral) {
    var manufacturerData = peripheral.advertisement.manufacturerData;
    ...
    console.log(manufacturerData.readUInt8(15))
